...
class LabelSmoothingLoss(nn.Module):
    ...
    def forward(self, logits: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        # Mask out padding positions
        mask = target.ne(self.padding_idx)  # (N,)
        if mask.sum().item() == 0:
            return log_probs.sum() * 0.0
        ...
        # NLL on the true target
        nll_loss = -log_probs.gather(1, target.unsqueeze(1)).squeeze(1)  # (M,)

        # Smooth loss: mean over all non-pad vocab positions
        # Zero out the pad column so it doesn't contribute
        smooth_log_probs = log_probs.clone()
        smooth_log_probs[:, self.padding_idx] = 0.0
        smooth_loss = -smooth_log_probs.sum(dim=1) / (self.vocab_size - 1)  # (M,)

        # Combine
        loss = self.confidence * nll_loss + self.smoothing * smooth_loss
        return loss.mean()  # mean over non-pad tokens